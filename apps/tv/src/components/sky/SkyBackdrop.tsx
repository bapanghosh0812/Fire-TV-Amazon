...
import { THEMES, SkyPhase, resolvePhase } from './phases';
...
/** Current sky phase, following the clock (re-checked every minute). */
...
type Scrim = 'none' | 'left' | 'center' | 'top';

/**
 * The living sky behind every screen: a big glowing sun by day, a detailed moon
 * and a sky full of stars by night, drifting clouds and layered mountains for
 * depth. Everything is vector, so it stays razor sharp on 4K TVs.
 */
export function SkyBackdrop({ scrim = 'none' }: { scrim?: Scrim }) {
...
function ScrimLayer({ kind }: { kind: Scrim }) {
  if (kind === 'none') return null;
  if (kind === 'left')
    return (
      <LinearGradient
        colors={['rgba(4,6,24,0.82)', 'rgba(4,6,24,0.5)', 'rgba(4,6,24,0)']}
  ...
  if (kind === 'top')
    return <LinearGradient colors={['rgba(4,6,24,0.7)', 'rgba(4,6,24,0)']} locations={[0, 0.35]} style={StyleSheet.absoluteFill} />;
  return <View style={[StyleSheet.absoluteFill, { backgroundColor: 'rgba(4,6,24,0.42)' }]} />;
}
